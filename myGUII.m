...
% Last Modified by GUIDE v2.5 23-May-2019 17:31:17
...
% Make the axes blend in the background
set(handles.axes1,'Color',[.94 .94 .94])

% get The latest created simulation struct to simulate
...
% Fill the memory address field locations on the GUI
for i = 0 : size(handles.ram.building,1)-1
    handles.ram.fillLocation(i+1,1,strcat('0x',dec2hex(i,2)));
end

% Pain the Register title location on the GUI
...
% Paint the register names locations on the GUI
...
% write the Supported Instructions structure locations
...
% paint the Executed instruction labels
...
% Initialize the instruction location
handles.IR_Value= block(1,1,.25,.9,.19,.05);
handles.IR_Value.fillLocation(1,1,'none');
handles.IR_Value.building(1,1).txtObj.FontSize=20;

% paint the instruction description location
...
% paint the addressing mode title location
...
% paint the addressing mode value location
...
% paint the addressing  mode description location
...
% Experimental future animation methods
% pause(2)
...
% Loop for every instruction in the simulationStruct
    for instructionNum=1:length(handles.simulationStruct.IR)
            ...
            % Controls the displayed memory locations on the GUI
            pivot=round(handles.memorySlider.Value);
            count=0;
            for i =  pivot:pivot+handles.NumberOfDisplayedMemoryLocations-1
                handles.ram.fillLocation(handles.NumberOfDisplayedMemoryLocations-count,2,strcat('0x',dec2hex(handles.simulationStruct.memory(handles.MemorySize-i,instructionNum),2)));
                count=count+1;
            end
            
            % Controls the displayed register location values
            ...
            % updates the slider location with the Execution of each
            % instruction
            ...
            % make the GUI go to the latest Execution state
            if handles.jmp2End.Value==1
            break;
            end
            % Controls the animation display mode according to the POPUP
            % menu chosen value
...
count=0;
for i =  pivot:pivot+handles.NumberOfDisplayedMemoryLocations-1
    handles.ram.fillLocation(handles.NumberOfDisplayedMemoryLocations-count,1,strcat('0x',dec2hex(handles.MemorySize-i-1,2)));
    handles.ram.fillLocation(handles.NumberOfDisplayedMemoryLocations-count,2,strcat('0x',dec2hex(handles.simulationStruct.memory(handles.MemorySize-i,end),2)));
    count=count+1;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
msgbox('Please wait until simulation object is created then the color of the axes would change','Plaeas be patient!')
...
% --- Otherwise, executes on mouse press in 5 pixel border or over jmp2End.
function jmp2End_CreateFcn(hObject, eventdata, handles)
